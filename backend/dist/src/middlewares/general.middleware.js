...
const general_validator_1 = __importDefault(
  require('../validators/general.validator'),
)
...
GeneralMiddleware.authorizationBrinquedos = (req, res, next) => {
  ...
  if (
    !token ||
    (token.funcao !== prisma_client_2.Funcao.ALMOXARIFE &&
      token.funcao !== prisma_client_2.Funcao.GERENTE)
  ) {
    res
      .status(403)
...
GeneralMiddleware.authorizationClientes = (req, res, next) => {
  ...
  if (
    !token ||
    (token.funcao !== prisma_client_2.Funcao.ANALISTA_CADASTRO &&
      token.funcao !== prisma_client_2.Funcao.GERENTE)
  ) {
    res
      .status(403)
...
GeneralMiddleware.authorizationLocacoes = (req, res, next) => {
  ...
  if (
    !token ||
    (token.funcao !== prisma_client_2.Funcao.AGENTE_LOCACAO &&
      token.funcao !== prisma_client_2.Funcao.GERENTE)
  ) {
    res
      .status(403)
...
GeneralMiddleware.authorizationPagamentos = (req, res, next) => {
  ...
  if (
    !token ||
    (token.funcao !== prisma_client_2.Funcao.CAIXA &&
      token.funcao !== prisma_client_2.Funcao.GERENTE)
  ) {
