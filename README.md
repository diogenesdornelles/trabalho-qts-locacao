...
| **RNF03** | O sistema deve oferecer uma interface intuitiva, seguindo diretrizes reconhecidas de usabilidade. Para melhorar a experiência do usuário, a interface deve:<br> - Tentar priorizar a utilização de **listas de seleção** para facilitar a inserção de dados.<br> - Exibir **notificações sobre campos inválidos** e fornecer sugestões de preenchimento.<br> - Seguir, na medida do possível, um **padrão de design visual** harmonioso para garantir consistência estética. | Usuário final | - |
...
| **RNF05** | O sistema deve garantir a **privacidade e segurança dos dados** dos usuários por meio das seguintes medidas:<br> - **Criptografia de armazenamento** usando, por exemplo, **criptografia Bcrypt** para proteção de senhas.<br> - **Restrições de acesso** baseadas em regras de permissão.<br> - Implementação de **UUIDs (Identificadores Únicos Universais)** para a identificação de recursos empresariais, garantindo rastreabilidade e segurança. | Administrador, Usuário final | RNF04, RNF06 |
